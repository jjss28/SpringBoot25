...
public class CustomSocialLoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, 
        ...
        || passwordEncoder.matches("1111", memberSecurityDTO.getMpw()))) {
            ...
            // 자동가입된 사용자기 때문에 암호 변경을 시킨다.

            log.info("회원 수정페이지로 이동합니다.....");
            response.sendRedirect("/member/modify"); // 컨트롤러 생성필수
    ...
    //                       implements AuthenticationSuccessHandler 시큐리티에 내장된 인터페이
    // 카카오톡으로 로그인 성공시 해야되는 일!!!
    // 패스워드가 1111로 되어있기 때문에 회원수정 페이지로 이동!!!
    // 암호변경하게 셋팅
}
